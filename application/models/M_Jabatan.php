...
class M_Jabatan extends CI_Model {
    ...
    private $TABLE = "tbl_mstjabatan";
    private $PK = "idJabatan";
    private $VIEW_TABLE = 'vw_mstjabatan';
     ...
     */
    public function getJabatan($key = "getAll", $kolom = null, $value_kolom = null, $value_pk = null, $json = true) {
        if ($key === "getAll") {
            $parsedBody['idUser'] = encode_str($_SESSION['idUser']);
            $Fetch = $this->guzzle->API_Get('B_Jabatan/getAll', $parsedBody, $json);
            ...
            $parsedBody['idUser'] = encode_str($_SESSION['idUser']);
            $Fetch = $this->guzzle->API_Get('B_Jabatan/getData/'.$value_pk, $parsedBody);
            if ($Fetch == $this->Code_UNAUTHORIZED) {
                return $this->Code_UNAUTHORIZED;
            } if ($Fetch !== $this->Code_UNAUTHORIZED) {
                return $Fetch;
     ...
     * $return  :   $RETURN_VALUE
     */
    public function saveJabatan($DATA_POST = null) {
        /*
        ...
        if (empty($DATA_POST)) {
            ...
            if (isset($DATA_POST['statusJabatan'])) {
                $statusJabatan = true;
            } else if (!isset($DATA_POST['statusJabatan'])) {
                $statusJabatan = false;
            }
            /*
             * Update data jabatan
            ...
            if (isset($DATA_POST[$this->PK])) {
                $data = array(
                    ...
                    'namaJabatan' => $DATA_POST['namaJabatan'],
                    'singkatanJabatan' => $DATA_POST['singkatanJabatan'],
                ...
                $this->guzzle->API_Put('B_Jabatan/put/', $data);

                $RETURN_VALUE = array(
                    "STATUS" => "Update",
                    "PESAN" => "Position ".$DATA_POST['namaJabatan']." successfuly updated",
            ...
            if (empty($DATA_POST[$this->PK])) {
                ...
                $this->guzzle->API_Post('B_Jabatan/post/', $data);

                $RETURN_VALUE = array(
                    "STATUS" => "Create",
                    "PESAN" => "Position ".$DATA_POST['namaJabatan']." successfuly created",
     ...
     * $return  :   $RETURN_VALUE
     *
     * Note     :   $DATA_POST wajib berisi Primary Key dan Nama Jabatan
     */
    public function deleteJabatan($DATA_POST = null) {
        /*
         * Bila $DATA_POST kosong maka akan mengembalikan data ke
         * controller dengan isi pesan pada $RETURN_VALUE
         * Bertujuan untuk pengecekan data.
        ...
        if (empty($DATA_POST)) {
            ...
            $this->guzzle->API_Delete('B_Jabatan/delete/', $id);

            $RETURN_VALUE = array(
                "STATUS" => "Delete",
                "PESAN" => "Position ".$DATA_POST['namaJabatan']." successfuly deleted",
                "PK" => encode_str($DATA_POST[$this->PK])
            );
            return $RETURN_VALUE;
        }
