...
class M_Referensi extends CI_Model {
    ...
    private $PK = "idReferensi";
    private $Code_UNAUTHORIZED = '401';

    /*
     * Alasan kenapa M_Referensi tidak ada Table View dikarenakan Tabel nya berdiri sendiri tanpa
     * "Foreign Key",
     ...
     */
    public function getReferensi($key = "getAll", $kolom = null, $value_kolom = null, $value_pk = null, $json = true) {
        if ($key === "getAll") {
            $parsedBody['idUser'] = encode_str($_SESSION['idUser']);
            $Fetch = $this->guzzle->API_Get('B_Referensi/getAll', $parsedBody, $json);
            ...
            $parsedBody['idUser'] = encode_str($_SESSION['idUser']);
            $Fetch = $this->guzzle->API_Get('B_Referensi/getData/'.$value_pk, $parsedBody);
            if ($Fetch == $this->Code_UNAUTHORIZED) {
                return $this->Code_UNAUTHORIZED;
            } if ($Fetch !== $this->Code_UNAUTHORIZED) {
                return $Fetch;
            }
        } if ($key === "getDataByJudul") {
            $parsedBody['idUser'] = encode_str($_SESSION['idUser']);
            $Fetch = $this->guzzle->API_Get('B_Referensi/getData/'.$value_pk.'/idJudul', $parsedBody);
            if ($Fetch == $this->Code_UNAUTHORIZED) {
                return $this->Code_UNAUTHORIZED;
            } if ($Fetch !== $this->Code_UNAUTHORIZED) {
                return $Fetch;
            ...
            self::getReferensi("getAll");
        }
    }

    /*
     * Fungsi   :   saveReferensi
     * Tipe     :   Public
     ...
     * $return  :   $RETURN_VALUE
     */
    public function saveReferensi($DATA_POST = null) {
        /*
        ...
        if (empty($DATA_POST)) {
            ...
            if (isset($DATA_POST[$this->PK])) {
                $data = array(
                    ...
                    'idPersyaratan' => $DATA_POST['idPersyaratan'],
                    'deskripsiReferensi' => $DATA_POST['deskripsiReferensi'],
                ...
                $this->guzzle->API_Put('B_Referensi/put/', $data);

                $RETURN_VALUE = array(
                    "STATUS" => "Update",
                    "PESAN" => "Referensi ".$DATA_POST['deskripsiReferensi']." successfuly updated",
                    "PK" => encode_str($DATA_POST['idJudul'])
                );
            }

            /*
             * Create data referensi
             */
            if (empty($DATA_POST[$this->PK])) {
                ...
                $this->guzzle->API_Post('B_Referensi/post/', $data);

                $RETURN_VALUE = array(
                    "STATUS" => "Create",
                    "PESAN" => "Referensi ".$DATA_POST['deskripsiReferensi']." successfuly created",
                    "PK" => encode_str($DATA_POST['idJudul'])
                );
            }
     ...
     * $return  :   $RETURN_VALUE
     *
     * Note     :   $DATA_POST wajib berisi Primary Key dan Nama Referensi
     */
    public function deleteReferensi($DATA_POST = null) {
        /*
         * Bila $DATA_POST kosong maka akan mengembalikan data ke
         * controller dengan isi pesan pada $RETURN_VALUE
         * Bertujuan untuk pengecekan data.
        ...
        if (empty($DATA_POST)) {
            ...
            /*
             * Delete data referensi
             */
            $id[$this->PK] = $DATA_POST[$this->PK];
            $this->guzzle->API_Delete('B_Referensi/delete/', $id);

            $RETURN_VALUE = array(
                "STATUS" => "Delete",
                "PESAN" => "Referensi ".$DATA_POST['deskripsiReferensi']." successfuly deleted",
                "PK" => encode_str($DATA_POST['idJudul'])
            );
            return $RETURN_VALUE;
        }
