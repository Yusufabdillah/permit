...
class M_Tipe extends CI_Model {
    ...
    private $TABLE = "tbl_msttipe";
    private $PK = "idTipe";
    private $Code_UNAUTHORIZED = '401';

    /*
     * Alasan kenapa M_Tipe tidak ada Table View dikarenakan Tabel nya berdiri sendiri tanpa
     * "Foreign Key",
     ...
     */
    public function getTipe($key = "getAll", $kolom = null, $value_kolom = null, $value_pk = null, $json = true) {
        if ($key === "getAll") {
            $parsedBody['idUser'] = encode_str($_SESSION['idUser']);
            $Fetch = $this->guzzle->API_Get('B_Tipe/getAll', $parsedBody, $json);
            ...
            $parsedBody['idUser'] = encode_str($_SESSION['idUser']);
            $Fetch = $this->guzzle->API_Get('B_Tipe/getData/'.$value_pk, $parsedBody);
            if ($Fetch == $this->Code_UNAUTHORIZED) {
                return $this->Code_UNAUTHORIZED;
            } if ($Fetch !== $this->Code_UNAUTHORIZED) {
                return $Fetch;
     ...
     * $return  :   $RETURN_VALUE
     */
    public function saveTipe($DATA_POST = null) {
        /*
        ...
        if (empty($DATA_POST)) {
            ...
            if (isset($DATA_POST[$this->PK])) {
                $data = array(
                    ...
                    'namaTipe' => $DATA_POST['namaTipe'],
                    'deskripsiTipe' => $DATA_POST['deskripsiTipe'],
                ...
                $this->guzzle->API_Put('B_Tipe/put/', $data);

                $RETURN_VALUE = array(
                    "STATUS" => "Update",
                    "PESAN" => "Type Document ".$DATA_POST['namaTipe']." successfuly updated",
            ...
            if (empty($DATA_POST[$this->PK])) {
                ...
                $this->guzzle->API_Post('B_Tipe/post/', $data);

                $RETURN_VALUE = array(
                    "STATUS" => "Create",
                    "PESAN" => "Type document ".$DATA_POST['namaTipe']." successfuly created",
     ...
     * $return  :   $RETURN_VALUE
     *
     * Note     :   $DATA_POST wajib berisi Primary Key dan Nama Tipe
     */
    public function deleteTipe($DATA_POST = null) {
        /*
         * Bila $DATA_POST kosong maka akan mengembalikan data ke
         * controller dengan isi pesan pada $RETURN_VALUE
         * Bertujuan untuk pengecekan data.
        ...
        if (empty($DATA_POST)) {
            ...
            $this->guzzle->API_Delete('B_Tipe/delete/', $id);

            $RETURN_VALUE = array(
                "STATUS" => "Delete",
                "PESAN" => "Type document ".$DATA_POST['namaTipe']." successfuly deleted",
                "PK" => encode_str($DATA_POST[$this->PK])
            );
            return $RETURN_VALUE;
        }
