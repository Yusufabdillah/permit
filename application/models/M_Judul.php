...
class M_Judul extends CI_Model {
     ...
     */
    public function getJudul($key = "getAll", $kolom = null, $value_kolom = null, $value_pk = null, $json = true) {
        if ($key === "getAll") {
            $parsedBody['idUser'] = encode_str($_SESSION['idUser']);
            $Fetch = $this->guzzle->API_Get('B_Judul/getAll', $parsedBody, $json);
            ...
            $parsedBody['idUser'] = encode_str($_SESSION['idUser']);
            $Fetch = $this->guzzle->API_Get('B_Judul/getData/'.$value_pk, $parsedBody);
            if ($Fetch == $this->Code_UNAUTHORIZED) {
                return $this->Code_UNAUTHORIZED;
            } if ($Fetch !== $this->Code_UNAUTHORIZED) {
                return $Fetch;
            }
        } if ($key === "getDataByKlasifikasi") {
            $parsedBody['idUser'] = encode_str($_SESSION['idUser']);
            $Fetch = $this->guzzle->API_Get('B_Judul/getData/'.$value_pk.'/idKlasifikasi', $parsedBody);
            if ($Fetch == $this->Code_UNAUTHORIZED) {
                return $this->Code_UNAUTHORIZED;
            } if ($Fetch !== $this->Code_UNAUTHORIZED) {
                return $Fetch;
     ...
     * $return  :   $RETURN_VALUE
     */
    public function saveJudul($DATA_POST = null) {
        /*
        ...
        if (empty($DATA_POST)) {
            ...
            if (isset($DATA_POST[$this->PK])) {
                $data = array(
                    ...
                    'idKlasifikasi' => $DATA_POST['idKlasifikasi'],
                    'namaJudul' => $DATA_POST['namaJudul'],
                    'rekom_syaratJudul' => $DATA_POST['rekom_syaratJudul'],
                ...
                $this->guzzle->API_Put('B_Judul/put/', $data);

                $RETURN_VALUE = array(
                    "STATUS" => "Update",
                    "PESAN" => "Judul ".$DATA_POST['namaJudul']." successfuly updated",
                    "PK" => encode_str($DATA_POST[$this->PK]),
                    "REDIRECT" => $DATA_POST['redirectKe']
                );
            }
            ...
            if (empty($DATA_POST[$this->PK])) {
                ...
                $this->guzzle->API_Post('B_Judul/post/', $data);

                $RETURN_VALUE = array(
                    "STATUS" => "Create",
                    "PESAN" => "Judul ".$DATA_POST['namaJudul']." successfuly created",
                );
            }
     ...
     * $return  :   $RETURN_VALUE
     *
     * Note     :   $DATA_POST wajib berisi Primary Key dan Nama Judul
     */
    public function deleteJudul($DATA_POST = null) {
        /*
         * Bila $DATA_POST kosong maka akan mengembalikan data ke
         * controller dengan isi pesan pada $RETURN_VALUE
         * Bertujuan untuk pengecekan data.
        ...
        if (empty($DATA_POST)) {
            ...
            $this->guzzle->API_Delete('B_Judul/delete/', $id);

            $RETURN_VALUE = array(
                "STATUS" => "Delete",
                "PESAN" => "Judul ".$DATA_POST['namaJudul']." successfuly deleted",
                "PK" => encode_str($DATA_POST[$this->PK])
            );
            return $RETURN_VALUE;
        }
