...
class M_SSKategori extends CI_Model {

    private $PK = "idSub_subk";
    private $Code_UNAUTHORIZED = '401';

    /*
     * Fungsi   :   getSSKategori
     * Tipe     :   Public
     ...
     */
    public function getSSKategori($key = "getAll", $kolom = null, $value_kolom = null, $value_pk = null, $json = true) {
        if ($key === "getAll") {
            $parsedBody['idUser'] = encode_str($_SESSION['idUser']);
            $Fetch = $this->guzzle->API_Get('B_SSKategori/getAll', $parsedBody, $json);
            ...
            $parsedBody['idUser'] = encode_str($_SESSION['idUser']);
            $Fetch = $this->guzzle->API_Get('B_SSKategori/getData/'.$value_pk, $parsedBody);
            if ($Fetch == $this->Code_UNAUTHORIZED) {
                return $this->Code_UNAUTHORIZED;
            } if ($Fetch !== $this->Code_UNAUTHORIZED) {
                return $Fetch;
            ...
            $parsedBody['idUser'] = encode_str($_SESSION['idUser']);
            $Fetch = $this->guzzle->API_Get('B_SSKategori/getData/'.$value_pk.'/idSubk', $parsedBody);
            if ($Fetch == $this->Code_UNAUTHORIZED) {
                return $this->Code_UNAUTHORIZED;
            } if ($Fetch !== $this->Code_UNAUTHORIZED) {
                return $Fetch;
            ...
            self::getSSKategori("getAll");
        }
    }

    /*
     * Fungsi   :   saveSSKategori
     * Tipe     :   Public
     ...
     * $return  :   $RETURN_VALUE
     */
    public function saveSSKategori($DATA_POST = null) {
        /*
        ...
        if (empty($DATA_POST)) {
            ...
            /*
             * Update data sub sub kategori
             */
            if (isset($DATA_POST[$this->PK])) {
                $data = array(
                    $this->PK => $DATA_POST[$this->PK],
                    'idSubk' => $DATA_POST['idSubk'],
                    'namaSub_subk' => $DATA_POST['namaSub_subk'],
                ...
                $this->guzzle->API_Put('B_SSKategori/put/', $data);

                $RETURN_VALUE = array(
                    "STATUS" => "Update",
                    "PESAN" => "Sub Sub Category ".$DATA_POST['namaSub_subk']." successfuly updated",
                    "PK" => encode_str($DATA_POST[$this->PK])
                );
            }

            /*
             * Create data sub sub kategori
             */
            if (empty($DATA_POST[$this->PK])) {
                ...
                $this->guzzle->API_Post('B_SSKategori/post/', $data);

                $RETURN_VALUE = array(
                    "STATUS" => "Create",
                    "PESAN" => "Sub sub category ".$DATA_POST['namaSub_subk']." successfuly created",
     ...
     */
    public function deleteSSKategori($DATA_POST = null) {
        /*
         * Bila $DATA_POST kosong maka akan mengembalikan data ke
         * controller dengan isi pesan pada $RETURN_VALUE
         * Bertujuan untuk pengecekan data.
        ...
        if (empty($DATA_POST)) {
            ...
            /*
             * Delete data sub sub kategori
             */
            $id[$this->PK] = $DATA_POST[$this->PK];
            $this->guzzle->API_Delete('B_SSKategori/delete/', $id);

            $RETURN_VALUE = array(
                "STATUS" => "Delete",
                "PESAN" => "Sub Sub Category ".$DATA_POST['namaSub_subk']." successfuly deleted",
                "PK" => encode_str($DATA_POST[$this->PK])
            );
            return $RETURN_VALUE;
        }
