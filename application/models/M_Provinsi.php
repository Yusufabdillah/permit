...
class M_Provinsi extends CI_Model {
    ...
    private $PK = "idProvinsi";
    private $VIEW_TABLE = 'vw_mstprovinsi';
     ...
     */
    public function getProvinsi($key = "getAll", $kolom = null, $value_kolom = null, $value_pk = null, $json = true) {
        if ($key === "getAll") {
            $parsedBody['idUser'] = encode_str($_SESSION['idUser']);
            $Fetch = $this->guzzle->API_Get('B_Provinsi/getAll', $parsedBody, $json);
            ...
            $parsedBody['idUser'] = encode_str($_SESSION['idUser']);
            $Fetch = $this->guzzle->API_Get('B_Provinsi/getData/'.$value_pk, $parsedBody);
            if ($Fetch == $this->Code_UNAUTHORIZED) {
                return $this->Code_UNAUTHORIZED;
            } if ($Fetch !== $this->Code_UNAUTHORIZED) {
                return $Fetch;
            }
        } if ($key === "getDataByNegara") {
            $parsedBody['idUser'] = encode_str($_SESSION['idUser']);
            $Fetch = $this->guzzle->API_Get('B_Provinsi/getData/'.$value_pk.'/idNegara', $parsedBody);
            if ($Fetch == $this->Code_UNAUTHORIZED) {
                return $this->Code_UNAUTHORIZED;
            } if ($Fetch !== $this->Code_UNAUTHORIZED) {
                return $Fetch;
     ...
     * $return  :   $RETURN_VALUE
     */
    public function saveProvinsi($DATA_POST = null) {
        /*
        ...
        if (empty($DATA_POST)) {
            ...
            if (isset($DATA_POST[$this->PK])) {
                $data = array(
                    ...
                    'namaProvinsi' => $DATA_POST['namaProvinsi'],
                ...
                $this->guzzle->API_Put('B_Provinsi/put/', $data);

                $RETURN_VALUE = array(
                    "STATUS" => "Update",
                    "PESAN" => "Province ".$DATA_POST['namaProv']." successfuly updated",
            ...
            if (empty($DATA_POST[$this->PK])) {
                ...
                $this->guzzle->API_Post('B_Provinsi/post/', $data);

                $RETURN_VALUE = array(
                    "STATUS" => "Create",
                    "PESAN" => "Province ".$DATA_POST['namaProvinsi']." successfuly created",
     ...
     */
    public function deleteProvinsi($DATA_POST = null) {
        /*
         * Bila $DATA_POST kosong maka akan mengembalikan data ke
         * controller dengan isi pesan pada $RETURN_VALUE
         * Bertujuan untuk pengecekan data.
        ...
        if (empty($DATA_POST)) {
            ...
            $this->guzzle->API_Delete('B_Provinsi/delete/', $id);

            $RETURN_VALUE = array(
                "STATUS" => "Delete",
                "PESAN" => "Province ".$DATA_POST['namaProvinsi']." successfuly deleted",
                "PK" => encode_str($DATA_POST[$this->PK])
            );
            return $RETURN_VALUE;
        }
