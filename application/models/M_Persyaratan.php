...
class M_Persyaratan extends CI_Model {
    ...
    private $TABLE = "tbl_mstpersyaratan";
    private $PK = "idPersyaratan";
    private $Code_UNAUTHORIZED = '401';

    /*
     * Alasan kenapa M_Persyaratan tidak ada Table View dikarenakan Tabel nya berdiri sendiri tanpa
     * "Foreign Key",
     * Bila dalam pengembangan kedepan ada menggunakan FK maka akan dibuatkan Tabel View nya
     */

    /*
     * Fungsi   :   getPersyaratan
     * Tipe     :   Public
     ...
     */
    public function getPersyaratan($key = "getAll", $kolom = null, $value_kolom = null, $value_pk = null, $json = true) {
        if ($key === "getAll") {
            $parsedBody['idUser'] = encode_str($_SESSION['idUser']);
            $Fetch = $this->guzzle->API_Get('B_Persyaratan/getAll', $parsedBody, $json);
            ...
            $parsedBody['idUser'] = encode_str($_SESSION['idUser']);
            $Fetch = $this->guzzle->API_Get('B_Persyaratan/getData/'.$value_pk, $parsedBody);
            if ($Fetch == $this->Code_UNAUTHORIZED) {
                return $this->Code_UNAUTHORIZED;
            } if ($Fetch !== $this->Code_UNAUTHORIZED) {
                return $Fetch;
            ...
            self::getPersyaratan("getAll");
        }
    }

    /*
     * Fungsi   :   savePersyaratan
     * Tipe     :   Public
     ...
     * $return  :   $RETURN_VALUE
     */
    public function savePersyaratan($DATA_POST = null) {
        /*
        ...
        if (empty($DATA_POST)) {
            ...
            /**
             * Jika ada suatu data memakai konsep status gunakan script dibawah
             * karena jika di view di non aktif kan slider nya maka data post = null
             * bukan tidak false, jadi harus di inisialisasi
             */
            if (isset($DATA_POST['statusOSSPersyaratan'])) {
                $statusOSSPersyaratan = true;
            } else if (!isset($DATA_POST['statusOSSPersyaratan'])) {
                $statusOSSPersyaratan = false;
            }
            ...
            if (isset($DATA_POST[$this->PK])) {
                $data = array(
                    ...
                    'namaPersyaratan' => $DATA_POST['namaPersyaratan'],
                    'statusOSSPersyaratan' => $statusOSSPersyaratan,
                ...
                $this->guzzle->API_Put('B_Persyaratan/put/', $data);

                $RETURN_VALUE = array(
                    "STATUS" => "Update",
                    "PESAN" => "Document Requirements ".$DATA_POST['namaPersyaratan']." successfuly updated",
                    "PK" => encode_str($DATA_POST[$this->PK])
                );
            }

            /*
             * Create data persyaratan
             */
            if (empty($DATA_POST[$this->PK])) {
                ...
                $this->guzzle->API_Post('B_Persyaratan/post/', $data);

                $RETURN_VALUE = array(
                    "STATUS" => "Create",
                    "PESAN" => "Document Requirements ".$DATA_POST['namaPersyaratan']." successfuly created",
     ...
     * $return  :   $RETURN_VALUE
     *
     * Note     :   $DATA_POST wajib berisi Primary Key dan Nama Persyaratan
     */
    public function deletePersyaratan($DATA_POST = null) {
        /*
         * Bila $DATA_POST kosong maka akan mengembalikan data ke
         * controller dengan isi pesan pada $RETURN_VALUE
         * Bertujuan untuk pengecekan data.
        ...
        if (empty($DATA_POST)) {
            ...
            $this->guzzle->API_Delete('B_Persyaratan/delete/', $id);

            $RETURN_VALUE = array(
                "STATUS" => "Delete",
                "PESAN" => "Document Requirements ".$DATA_POST['namaPersyaratan']." successfuly deleted",
                "PK" => encode_str($DATA_POST[$this->PK])
            );
            return $RETURN_VALUE;
        }
