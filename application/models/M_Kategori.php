...
class M_Kategori extends CI_Model {
     ...
     */
    public function getKategori($key = "getAll", $kolom = null, $value_kolom = null, $value_pk = null, $json = true) {
        if ($key === "getAll") {
            $parsedBody['idUser'] = encode_str($_SESSION['idUser']);
            $Fetch = $this->guzzle->API_Get('B_Kategori/getAll', $parsedBody, $json);
            ...
            $parsedBody['idUser'] = encode_str($_SESSION['idUser']);
            $Fetch = $this->guzzle->API_Get('B_Kategori/getData/'.$value_pk, $parsedBody);
            if ($Fetch == $this->Code_UNAUTHORIZED) {
                return $this->Code_UNAUTHORIZED;
            } if ($Fetch !== $this->Code_UNAUTHORIZED) {
                return $Fetch;
            }
        } if ($key === "getDataByAspek") {
            $parsedBody['idUser'] = encode_str($_SESSION['idUser']);
            $Fetch = $this->guzzle->API_Get('B_Kategori/getData/'.$value_pk.'/idAspek', $parsedBody);
            if ($Fetch == $this->Code_UNAUTHORIZED) {
                return $this->Code_UNAUTHORIZED;
            } if ($Fetch !== $this->Code_UNAUTHORIZED) {
                return $Fetch;
     ...
     * $return  :   $RETURN_VALUE
     */
    public function saveKategori($DATA_POST = null) {
        /*
        ...
        if (empty($DATA_POST)) {
            ...
            if (isset($DATA_POST[$this->PK])) {
                $data = array(
                    ...
                    'idAspek' => $DATA_POST['idAspek'],
                    'namaKategori' => $DATA_POST['namaKategori'],
                ...
                $this->guzzle->API_Put('B_Kategori/put/', $data);

                $RETURN_VALUE = array(
                    "STATUS" => "Update",
                    "PESAN" => "Category ".$DATA_POST['namaKategori']." successfuly updated",
            ...
            if (empty($DATA_POST[$this->PK])) {
                ...
                $this->guzzle->API_Post('B_Kategori/post/', $data);

                $RETURN_VALUE = array(
                    "STATUS" => "Create",
                    "PESAN" => "Category ".$DATA_POST['namaKategori']." successfuly created",
     ...
     * $return  :   $RETURN_VALUE
     *
     * Note     :   $DATA_POST wajib berisi Primary Key dan Nama Kategori
     */
    public function deleteKategori($DATA_POST = null) {
        /*
         * Bila $DATA_POST kosong maka akan mengembalikan data ke
         * controller dengan isi pesan pada $RETURN_VALUE
         * Bertujuan untuk pengecekan data.
        ...
        if (empty($DATA_POST)) {
            ...
            $this->guzzle->API_Delete('B_Kategori/delete/', $id);

            $RETURN_VALUE = array(
                "STATUS" => "Delete",
                "PESAN" => "Category ".$DATA_POST['namaKategori']." successfuly deleted",
                "PK" => encode_str($DATA_POST[$this->PK])
            );
            return $RETURN_VALUE;
        }
